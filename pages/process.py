    ...
    [
        dcc.Markdown(
            ...
            class ColumnSelector(BaseEstimator, TransformerMixin):
                \"\"\"
                Transformer to select a group of columns based on a list of features.
                \"\"\"
            ...
            oucatPipe = Pipeline( [
                ("ccol", ColumnSelector(categorical_oufeatures)),
                ("ckbe", SelectKBest(score_func=chi2, k=291))
                ] )


            ounumPipe = Pipeline( [
                ("ncol", ColumnSelector(numeric_oufeatures)),
                ("nkbe", SelectKBest(score_func=f_classif, k=90))
                ] )

            oufeats = FeatureUnion([('cats', oucatPipe), ('nums', ounumPipe)])
            ```
