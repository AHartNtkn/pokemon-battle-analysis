...
column1 = dbc.Col(
    [
        dcc.Markdown(
            ...
            The initial dataset used for training contains several thousand Pokemon battles pulled from Pokémon Showdown. It only contains the Pokémon on each team, along with the winner.
            ...
            This data is then processed into a more usable form. Most of the data wrangling and feature engineering was done in Mathematica. Starting, the train and test data are imported, processed, and combined;
            ...
            At this point, it's worth noting that we have a baseline of 50%. That is to say, since the team orders don't matter and are randomly assigned, about half the time team 1 will win and team 2 wins the other half. By guessing that team 1 wins every time, we already obtain a 50% prediction rate. Any useful model should be able to out-perform this.

            The actual model used here is fairly simple. The features are filtered using K-best using chi-squared scores for the one-hot encoded Pokémon and F-scores for the various team metrics.
            ...
            After that, the data is placed into a random forest model. As previously stated, each prediction is generated by running a team through the model twice, and taking the mean of the probabilities outputted by the model.
            ...
            In addition to this, whether a team is on team 1 or 2 doesn't matter since both players queue moves to be performed simultaneously. This can be accounted for by running each scenario through the model twice; the second time having the two teams switched.
