...
column1 = dbc.Col(
    [
        ...
        dcc.Markdown(
            ...
            Most limitations of the model stem from the dataset itself. Initially, it only contained the Pokemon on each team, along with the winner. From this, many team metrics can be generated, but this has its limits. The dataset doesn't contain information which is often essential for inferring the strategy underlying a teams construction. For example, it doesn't mention held items, moves, effort values, or special abilities. Each Pokemon comes in a multitude of variations which can dramatically effect the outcome of a battle.
            
            Perhaps even more import than the lack of information on individual Pokemon is the lack of information on the battlers. Having some kind of proxy for skill, such as some kind of Elo rating, would likely help improve accuracy more than any other individual metric. More speculatively, one might be able to render a short description of a team strategy into a vector using NLP, which might help improve accuracy.
